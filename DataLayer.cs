...
namespace HMS_test1
{
    class DataLayer
    {
        ...
        public DataTable newGetData(string SqlText, object[,] Parameters, string name)
        {
            DataTable dt = new DataTable();
            if (IsValid)
            {
        ...
        public object newGetValue(string SqlText, object[,] Parameters)
        {
            if ((IsValid) && (SqlText.Length > 0))
            {
                object v = null;
        ...
        public int newExecuteActionCommand(string CommandText, object[,] Parameters)
        {
            int rep = 0;
